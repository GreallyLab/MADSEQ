...
########################## function2: convert data to dataList for rjags ##########################
generateDataList_new = function(datalist,type="one"){
    BAF = datalist$z/datalist$N
    mixture = rep(NA,datalist$nSNP)
    m = datalist$m
    if (type=="one"){
        datalist$mixture = mixture
        Nclust = 2
    ...
    if (type=="two"){
        Nclust = 3
        mixture[match(max(BAF),BAF)]=1
        mixture[match(min(BAF),BAF)]=2
        datalist$mixture = mixture
        datalist$Nclust = Nclust}
...
########################## function3: write data to txt ##########################
createDataFile = function(file,dataList){
    sink(file)
    cat("nSNP<-")
    cat(dataList$nSNP)
    cat("\n")
    ...
    cat("N_cov<-c(")
    for (i in 1:(length(dataList$N_cov)-1)){
        cat(dataList$N_cov[i])
        cat(",")
    }
    cat(dataList$N_cov[length(dataList$N_cov)])
    cat(")\n")
    
    cat("z<-c(")
    for (i in 1:(length(dataList$z)-1)){
        cat(dataList$z[i])
        cat(",")
    }
    cat(dataList$z[length(dataList$z)])
    cat(")\n")
    
    cat("N<-c(")
    for (i in 1:(length(dataList$N)-1)){
        cat(dataList$N[i])
        cat(",")
    }
    cat(dataList$N[length(dataList$N)])
    cat(")\n")
    ...
    cat("mixture<-c(")
    for (i in 1:(length(dataList$mixture)-1)){
        cat(dataList$mixture[i])
        cat(",")
    }
    cat(dataList$mixture[length(dataList$mixture)])
    cat(")\n")
